  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((option) =>
      option
        .setName("input")
        .setDescription(
          "Steam profile link, custom URL, vanity name, or SteamID64",
        )
        .setRequired(true),
    ),
...
function saveLink(discordId, steamId, steamName, originalNickname = null) {
  ...
  const resolvedOriginalNickname =
    existing?.original_nickname ?? originalNickname ?? null;
...
function removeLink(discordId) {
  return db
    .prepare(`DELETE FROM steam_links WHERE discord_id = ?`)
    .run(discordId);
}
...
async function applyRoleAndNickname(
  guild,
  userId,
  steamName,
...
async function removeRoleAndRestoreNickname(
  guild,
  userId,
  ...
  try {
    const restored =
      originalNickname && originalNickname.trim() ? originalNickname : null;
...
client.once(Events.ClientReady, async () => {
  ...
  try {
    ...
    await logToChannel(
      `❌ Startup sync failed: ${escapeMarkdownLite(err.message)}`,
    );
  }
  ...
  setInterval(
    () => {
      syncSteamNames({ triggeredBy: "interval" }).catch(async (err) => {
        ...
        await logToChannel(
          `❌ Scheduled sync failed: ${escapeMarkdownLite(err.message)}`,
        );
      });
    },
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    ...
    if (interaction.commandName === "bulkimport") {
      ...
      const entries = parseBulkImportLines(rawData);

      if (!entries.length) {
        await interaction.editReply(
          "No valid lines were provided.\nUse one line per user:\n`DISCORD_USER_ID STEAM_ID64`\nor\n`@mention STEAM_ID64`",
        );
        return;
      }
      ...
      for (const entry of entries) {
        try {
          if (!entry.userId) {
            failCount++;
            results.push(
              `Line ${entry.lineNumber}: Invalid Discord mention or user ID.`,
            );
            continue;
          }
          ...
          if (!targetUser) {
            failCount++;
            results.push(
              `Line ${entry.lineNumber}: Could not find Discord user \`${entry.userId}\`.`,
            );
            continue;
          }
          ...
          if (
            existingSteamLink &&
            existingSteamLink.discord_id !== targetUser.id
          ) {
            failCount++;
            results.push(
              `Line ${entry.lineNumber}: Steam ID \`${profile.steamid}\` is already linked to another Discord account.`,
            );
            continue;
          }
          ...
          try {
            ...
            const existingTargetLink = getLinkByDiscordId(targetUser.id);
            originalNickname =
              existingTargetLink?.original_nickname ?? targetUser.username;
          }
          ...
          await logToChannel(
            `📥 ${interaction.user.tag} used /bulkimport on <@${targetUser.id}> → **${escapeMarkdownLite(profile.personaname)}** (\`${profile.steamid}\`).`,
          );
        } catch (err) {
          failCount++;
      ...
      const fullSummary = [summaryHeader, "", ...results].join("\n");

      if (fullSummary.length <= 1900) {
        await interaction.editReply(fullSummary);
      } else {
        await interaction.editReply(summaryHeader);
    ...
    if (interaction.commandName === "link") {
      ...
      if (
        existingSteamLink &&
        existingSteamLink.discord_id !== interaction.user.id
      ) {
        await interaction.editReply(
    ...
    if (interaction.commandName === "mylink") {
      ...
      await interaction.editReply(
        formatLinkInfo(interaction.user.id, existing),
      );
      return;
    }
    ...
    if (interaction.commandName === "adminunlinksteam") {
      ...
      if (!existing) {
        await interaction.editReply(
          "That user does not have a linked Steam account.",
        );
        return;
      }
    ...
    try {
      await logToChannel(
        `❌ Command error: ${escapeMarkdownLite(error.message)}`,
      );
